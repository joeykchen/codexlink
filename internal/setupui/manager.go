...
func (m *Manager) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	if !ok {
		...
		if len(parts) == 2 {
			writeStatusJSON(response, http.StatusGone, Status{State: StateWaiting})
			return
		}
